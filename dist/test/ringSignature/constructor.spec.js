...
describe("Test Constructor", () => {
    ...
    describe("Test constructor with invalid parameters", () => {
        ...
        it("Should throw if curve is invalid", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_secp256k1, data.randomC, data.randomResponses, new src_1.Curve("invalid name"))).toThrow((0, errors_1.unknownCurve)("invalid name"));
        });
        /* -------------TEST CONFIG.HASH------------- */
        it("Should pass if config.hash is keccack256", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_secp256k1, data.randomC, data.randomResponses, secp256k1, { hash: hashFunction_1.HashFunction.KECCAK256 }));
        });
        it("Should pass if config.hash is sha512", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_secp256k1, data.randomC, data.randomResponses, secp256k1, { hash: hashFunction_1.HashFunction.SHA512 }));
        });
