...
describe("test checkRing()", () => {
    ...
    it("Should throw an error if the ring contains invalid points", () => {
        ...
        invalid.x = BigInt(0);
        expect(() => (0, ringSignature_1.checkRing)(data.publicKeys_secp256k1.concat([invalid]), secp256k1)).toThrow("Invalid point: At least one point is not valid: Error: Invalid point: not on curve");
    });
});
